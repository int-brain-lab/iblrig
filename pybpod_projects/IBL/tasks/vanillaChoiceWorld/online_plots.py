...
# @Last Modified by:   N
# @Last Modified time: 2018-03-06 11:33:20
...
def get_barplot_data(trial_data, target_reward):
    out = {}
    ...
    out['water_delivered'] = out['ntrials_correct'] * target_reward
    out['session_name'] = trial_data['data_file_path'].split('/')[-2]

    t = dateutil.parser.parse(out['session_name'].replace('_', ':'))
    out['time_from_start'] = (datetime.datetime.now() - t)
...
def plot_bars(trial_data, target_reward, ax=None):
    if ax is None:
        # f = plt.figure()  # figsize=(19.2, 10.8), dpi=100)
        ax = plt.subplot2grid((1, 1), (0, 0), rowspan=1, colspan=1)
    ax.cla()

    bar_data = get_barplot_data(trial_data, target_reward)
    ...
    def make_bar_texts(ax, ypos, vars):
        ...
        for var in vars:
            ...
            ax.text(left + (var * .15), ypos - 0.33, str(left), color='black',
    ...
    ax.text(max(y) / 10, 3 - .125, str(bar_data['time_from_start']),
    ...
    ax.text(bar_data['water_delivered'] + 1, 0 - .33,
...
def parse_trial_data(trial_data):
    out = json.laods(trial_data)
    return out


if __name__ == '__main__':
    data_file = '/home/nico/Projects/IBL/IBL-github/pybpod_projects/IBL/data/\
vCW_test_mouse/2018-02-17T18:56:08.851608/pycw_vanilla.data.json'
    ...
    plot_bars(trial_data, 3, ax=ax_bars)
    psyfun_df = update_psyfun_df(trial_data, psyfun_df)
