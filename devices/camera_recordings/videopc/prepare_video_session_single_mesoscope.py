...
def main(mouse: str, training_session: bool = False, new: bool = False) -> None:
    ...
    assert DATA_FOLDER == (tmp := get_local_and_remote_paths().remote_subjects_folder), f'{DATA_FOLDER} does not equal {tmp}'
    ...
    os.chdir(here)
    # Check lengths
    len_files(session_path, display=True)  # Will printout the results
