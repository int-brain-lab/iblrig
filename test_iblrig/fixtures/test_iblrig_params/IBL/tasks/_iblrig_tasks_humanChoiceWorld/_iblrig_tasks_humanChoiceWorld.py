...
# =============================================================================
    ...
    sma.add_state(
        ...
        output_actions=[('Valve1', 255), ('SoftCode', 4)])
    ...
    pct = np.mean(tph.trial_correct_buffer)
    mrt = np.median(tph.response_time_buffer)
    msg = f"\nCurrent performance: {pct}\nCurrent median RT:   {mrt} (s)\nCurrent score:       {(pct * 100) / mrt}"
    log.error(msg)
bpod.close()
