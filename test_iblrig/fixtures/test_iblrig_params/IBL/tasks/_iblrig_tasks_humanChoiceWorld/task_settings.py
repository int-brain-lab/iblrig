...
# REWARDS
AUTOMATIC_CALIBRATION = False  # Wether to look for a calibration session and func to define the valve opening time  # noqa
CALIBRATION_VALUE = 0.067  # calibration value for 3ul of target reward amount (ignored if automatic ON)  # noqa
REWARD_AMOUNT = 0  # (µl) Amount of reward to be delivered upon correct choice each trial (overwitten if adaptive ON)  # noqa
REWARD_TYPE = 'None'  # Water, Water 10% Sucrose, Water 15% Sucrose, Water 2% Citric Acid (Guo et al.. PLoS One 2014)  # noqa
# TASK
NTRIALS = 120  # Number of trials for the current session
...
QUIESCENT_PERIOD = 0  # + x, where x~exp(0.35), t ∈ 0.2 <= R <= 0.5
INTERACTIVE_DELAY = 0.0  # (s) how long after stim onset the CL starts
RESPONSE_WINDOW = 10  # Time to move the wheel after go tone (seconds)
...
GO_TONE_FREQUENCY = 3000  # 5KHz
GO_TONE_AMPLITUDE = 0.05  # [0->1] 0.0272 for 70dB SPL Xonar
# POOP COUNT LOGGING
