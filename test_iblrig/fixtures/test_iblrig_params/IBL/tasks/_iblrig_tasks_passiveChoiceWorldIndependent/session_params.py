...
class SessionParamHandler(object):
    ...
    def load_passive_session_delays_ids(self) -> tuple:
        ...
        passive_session_folder = base.joinpath(self.PYBPOD_PROJECT, "tasks", self.PYBPOD_PROTOCOL,
                                               "sessions")
        stimDelays = np.load(
            passive_session_folder / f"session_{pregenerated_session_num}_passive_stimDelays.npy")
        stimIDs = np.load(
            passive_session_folder / f"session_{pregenerated_session_num}_passive_stimIDs.npy")
...
if __name__ == "__main__":
    ...
    _user_settings.PYBPOD_PROTOCOL = "_iblrig_tasks_passiveChoiceWorldIndependent"
    if platform == "linux":
