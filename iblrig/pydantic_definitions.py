...
class HardwareSettingsSound(BunchModel):
    ...
    DEFAULT_CHANNELS: Literal['stereo', 'left', 'right'] = 'stereo'
    # ATTENUATION_DB: float = Field(default=0, le=0)


class HardwareSettingsValve(BunchModel):
    WATER_CALIBRATION_DATE: date
    WATER_CALIBRATION_RANGE: list[PositiveFloat] = Field(min_length=2, max_length=2)
    WATER_CALIBRATION_N: PositiveInt = Field(ge=3, default=5)
    WATER_CALIBRATION_OPEN_TIMES: list[PositiveFloat] = Field(min_length=2)
    WATER_CALIBRATION_WEIGHT_PERDROP: list[PositiveFloat] = Field(min_length=2)
